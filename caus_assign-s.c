...
static int *hit_rcdex,*hit_read1,*hit_read2,*hit_chrom,*hit_index,*hit_rcmap;
static long *hit_locus;
...
/*
    ...
    if((hit_locus = (long *)calloc(50,sizeof(long))) == NULL)
    {
      printf("fmate: calloc - hit_chrom\n");
      exit(1);
    }
    if((hit_chrom = (int *)calloc(50,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_chrom\n");
      exit(1);
    }
    if((hit_index = (int *)calloc(50,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_chrom\n");
      exit(1);
    }
    if((hit_rcmap = (int *)calloc(50,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_chrom\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %d %d",R_Name[i],tempc1,&hit_read1[i],&hit_rcdex[i],&hit_read2[i])!=EOF)
    {
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
	  ...
	  for(n=i;n<j;n++)
	  {
          ...
          ArraySort2_Int2(50,hit_chrom,hit_index);
          idd = hit_index[0]; 
          for(n=i;n<j;n++)
          {
             if(hit_read1[n] == idd)
             {
               if(hit_rcdex[n] == 1)
                 rc_index++;
               sum_loci = sum_loci + hit_read2[n];
               max_hit++;
             }
          }
          if(max_hit>0)
            hit_loci = sum_loci/max_hit;
          else
            hit_loci = 0;
          rate = rc_index;
          rate = rate/hit_chrom[0];
          if(rate >= 0.5)
            rcd = 1;
          else
            rcd = 0;
          rate = kk;
          rate = hit_chrom[0]/rate;
          idd = atoi(R_Name[i]);
//	  for(n=0;n<30;n++)
//            printf("www: %d %d %d %d\n",n,kk,hit_chrom[n],hit_index[n]);
          if((hit_chrom[0]>=10)&&(rate > 0.4))
            fprintf(namef,"scaff2chr: %d %d %d %d %d %d %f || %d %d %d\n",idd,j-i,hit_index[0],hit_chrom[0],hit_index[1],hit_chrom[1],rate,rc_index,rcd,hit_loci);
        }
        else
        {

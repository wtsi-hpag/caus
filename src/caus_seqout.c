...
int main(int argc, char **argv)
{
    ...
    int i,j,nSeq,args,i_contig,idt,stopflag,num_hit,n_scaff,nRead,n_place,n_chrs;
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_scaffold_fastq> <assign-contig_file> <output_scaffold_fastq> \n",argv[0]);
      exit(1);
    }
...
/*  input read alignment info line   */
    ...
    if((namef = fopen(argv[args+4],"w")) == NULL)
    {
      printf("ERROR main:: reads group file: %s \n",argv[args]);
      exit(1);
    }
...
//         fprintf(fpOutfast,"@%s\n",seqp->name);
    ...
    fprintf(namef,"Placed   %d %ld %ld %f\n",n_place,seq_placed,seq_unplaced+seq_placed,rate);
    rate = seq_unplaced+seq_placed;
    rate = seq_unplaced/rate;
    fprintf(namef,"Unplaced %d %ld %f\n",n_scaff,seq_unplaced,rate);
    fclose(fpOutfast);
    fclose(namef);
    return EXIT_SUCCESS;

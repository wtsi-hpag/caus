...
/*
    ...
    if((pair_score = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - pair_score\n");
      exit(1);
    }
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
	  ...
	  for(n=i;n<j;n++)
	  {
	     ...
	     if((m-n) <= 3)
	     {
//                 printf("name: %s %s %d %d\n",DBname[n],S_Name[readIndex[n]],m-n,m);
	       for(k=n;k<m;k++)
	       {
	          int idk = readIndex[k];
		  if(superlength[idk] > 5000)
		    hit_mask[idk] = 1;
//                  printf("yyy: %s %s %d %d %d %d %d %d %d\n",R_Name[i],S_Name[idk],hit_read2[idk],readlength[idk],superlength[idk],i,idk,j-i,m-n);
	       }
	     }
	     n = m-1;
          }
